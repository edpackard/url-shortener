...
import ShortenerRoute from '../routers/ShortenUrl'
...
describe('ShortenerRoute', () => {
  ...
  it("should call UrlController.shorternUrl function when POST /", async() => {
    const body = { url: 'too-long-url'};
    await request.post(`/`).send(body);
    expect(UrlControllerMock.shortenUrl).toHaveBeenCalledWith(
